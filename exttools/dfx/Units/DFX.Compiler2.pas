...
uses
  DFX.Project, DFX.Compiler2.Types, DFX.Compiler.Helper, SysUtils, Classes,
  Windows, ShellAPI, Generics.Defaults, Generics.Collections, JclSysUtils,
  System.RegularExpressionsCore, D3DCommon_JSB, D3DCompiler_JSB, D3D11_JSB,
  TypInfo, JCLStrings, Math, IOUtils;

// Compiles a project
// Not thread-safe
procedure CompileDFXProject(Project: TDFXProject; Handler: IDFXCompilerHandler);
function GetShaderDASM(Project: TDFXProject; Handler: IDFXCompilerHandler; F: TDFXFunction; var DASM: string): Boolean;
...
begin
  ...
  // Implicit truncation of vector
  i := Pos('warning X3206', CompilerOutput);
  if i > 0 then
  begin
    Inc(i, Length('warning X'));
    s := Copy(CompilerOutput, i, Pos(':', CompilerOutput, i + 1) - i);
    Result.ErrorCode := StrToInt(s);
  end
  else
  begin
...
begin
  ...
  // GState.ShaderInclusions.Move(errorFileIndex, tarIdx);
      ...
      ptCompute:
        begin
          ...
          begin
            ...
            csoFile := GState.CppTranslator.UCppCompiledShaderFileName(f.FilePath('/'), f.ShaderProfile);
            CppTxt.Add(GState.CppTranslator.ULoadShader(Format('m_pass%dCS', [i]), csoFile, f.ShaderProfile));
          end;
        end;
      ptGeometry:
        begin
          ...
          begin
            ...
            csoFile := GState.CppTranslator.UCppCompiledShaderFileName(f.FilePath('/'), f.ShaderProfile);
            CppTxt.Add(GState.CppTranslator.ULoadShader(Format('m_pass%dVS', [i]), csoFile, f.ShaderProfile));
          end;
          ...
          begin
            ...
            csoFile := GState.CppTranslator.UCppCompiledShaderFileName(f.FilePath('/'), f.ShaderProfile);
            CppTxt.Add(GState.CppTranslator.ULoadShader(Format('m_pass%dPS', [i]), csoFile, f.ShaderProfile));
          end;

          if p.GeometryShader.Reference.Length > 0 then
          begin
            f := GState.Effect.FindGlobalFunctionReference(p.GeometryShader.Reference);
            csoFile := GState.CppTranslator.UCppCompiledShaderFileName(f.FilePath('/'), f.ShaderProfile);
            CppTxt.Add(GState.CppTranslator.ULoadShader(Format('m_pass%dGS', [i]), csoFile, f.ShaderProfile));
          end;
        end;
      else
        Assert(False);
    ...
    case p.PassType of
      ...
      ptGeometry:
        begin
          ...
          if p.GeometryShader.Reference.Length > 0 then
            CppAddDisposeShader(CppTxt, GState.CppTranslator.UUnloadShader(Format('m_pass%dGS', [i])));
        end;
      else
        Assert(False);
    end;
...
begin
  CppTxt.Add(Format('void dfx::%sinitialize(void *userData)', [GState.Effect.FilePath('::')]));
  CppTxt.Add('{');
  CppTxt.Indent;
...
begin
  ...
  begin
    ...
    // TODO: add subresource index
    CppTxt.Add(GState.CppTranslator.UBindResource(D3DCallStr, varName, i + Group.BindPoint, res.SubresourceIndex));
  end;
end;
...
begin
  ...
  begin
    ...
    varName := CppFinalizeVariableName(res);
    // TODO: add subresource index
    CppTxt.Add(GState.CppTranslator.UBindResource(D3DCallStr, 'nullptr', i + Group.BindPoint, res.SubresourceIndex));
  end;
end;
...
begin
  ...
  begin
    res := TDFXResource(Group.ResVarNames.Objects[i]);
    varName := CppFinalizeVariableName(res);
    CppTxt.Add(GState.CppTranslator.UBindUAV(D3DCallStr, varName, i + Group.BindPoint, res.SubresourceIndex, StrToInt(res.UAVInitialCount)));
  end;
end;
...
begin
  ...
  begin
    res := TDFXResource(Group.ResVarNames.Objects[i]);
    varName := CppFinalizeVariableName(res);
    CppTxt.Add(GState.CppTranslator.UBindUAV(D3DCallStr, 'nullptr', i + Group.BindPoint, res.SubresourceIndex, StrToInt(res.UAVInitialCount)));
  end;
end;
...
begin
  ...
  for i := 0 to Group.ResVarNames.Count - 1 do
  begin
    varName := CppFinalizeVariableName(TDFXResource(Group.ResVarNames.Objects[i]));
    CppTxt.Add(GState.CppTranslator.UBindSampler(D3DCallStr, varName, i + Group.BindPoint));
  end;
end;
...
begin
  ...
  for i := 0 to Group.ResVarNames.Count - 1 do
  begin
    varName := CppFinalizeVariableName(TDFXConstantBuffer(Group.ResVarNames.Objects[i]));
    CppTxt.Add(GState.CppTranslator.UBindConstants(D3DCallStr, varName, i + Group.BindPoint));
  end;
end;
...
procedure CppAddPassSet(CppTxt: TDFXNamespaceStringList;
  const D3DCallStr, ShaderShort: string; const Reflection: TDFXShaderReflectionData; PassIndex: Integer;
  BindUAVs: Boolean);
var
...
begin
  ...
  try
    ...
    CppAddShaderBinding(Reflection.Textures, CppTxt, D3DCallStr, group, CppAddBindTextures);

    if BindUAVs then
      CppAddShaderBinding(Reflection.UAVs, CppTxt, D3DCallStr, group, CppAddBindUAVs);

    CppAddShaderBinding(Reflection.SamplerStates, CppTxt, D3DCallStr, group, CppAddBindSamplerStates);
    CppAddShaderBinding(Reflection.ConstantBuffers, CppTxt, D3DCallStr, group, CppAddBindConstantBuffers);

    CppTxt.Add(GState.CppTranslator.UBindShader(D3DCallStr, 'm_pass' + IntToStr(PassIndex) + ShaderShort));
  finally
...
begin
  ...
  try
    // Set resources in adjacent groups
    CppAddShaderBinding(Reflection.Textures, CppTxt, D3DCallStr, group, CppAddUnbindTextures);

    if BindUAVs then
      CppAddShaderBinding(Reflection.UAVs, CppTxt, D3DCallStr, group, CppAddUnbindUAVs);
  finally
    group.ResVarNames.Free;
  end;
end;
...
begin
  if Assigned(Reflection.CS) then
    CppAddPassSet(CppTxt, 'compute_shader', 'CS', Reflection.CS, PassIndex, True)
  else
  begin
    if (Assigned(Reflection.VS) and (GState.Effect.Passes[PassIndex].VertexShader.Reference.Length > 0)) then
      CppAddPassSet(CppTxt, 'vertex_shader', 'VS', Reflection.VS, PassIndex, False);

    if (Assigned(Reflection.PS)) and (GState.Effect.Passes[PassIndex].PixelShader.Reference.Length > 0) then
      CppAddPassSet(CppTxt, 'pixel_shader', 'PS', Reflection.PS, PassIndex, False);

    if (Assigned(Reflection.GS) and (GState.Effect.Passes[PassIndex].GeometryShader.Reference.Length > 0)) then
      CppAddPassSet(CppTxt, 'geometry_shader', 'GS', Reflection.GS, PassIndex, False);
  end;
end;
...
begin
  // Shaders
  if not Assigned(Reflection[PassIndex].CS) then
  begin
    if Assigned(Reflection[PassIndex].VS) and (GState.Effect.Passes[PassIndex].VertexShader.Reference.Length > 0) then
      CppTxt.Add(GState.CppTranslator.UBindShader('vertex_shader', 'nullptr'));

    if Assigned(Reflection[PassIndex].PS) and (GState.Effect.Passes[PassIndex].PixelShader.Reference.Length > 0) then
      CppTxt.Add(GState.CppTranslator.UBindShader('pixel_shader', 'nullptr'));

    if Assigned(Reflection[PassIndex].GS) and (GState.Effect.Passes[PassIndex].GeometryShader.Reference.Length > 0) then
      CppTxt.Add(GState.CppTranslator.UBindShader('geometry_shader', 'nullptr'));
  end;
end;
...
begin
  ...
  CppAddResourceSemantics(CppTxt, Reflection.GS, 'GS', PassIndex);
end;
*}
...
begin
  ...
  CppTxt.Add('{');
  CppTxt.Indent;
  CppTxt.Add('on_begin_run(userData);');
  CppTxt.Add('');
  for i := 0 to GState.Effect.Passes.Count - 1 do
  begin
    ...
    case p.PassType of
      ...
      ptGeometry:
        begin
          ...
          if p.GeometryShader.Reference.Length > 0 then
            CppTxt.Add(Format('%s *m_pass%dGS;', [shaderClass, i]));
        end;
      else
        Assert(False);
    end;
...
var
  i: Integer;
  relativeBasePath, basePath, line, lastDir: string;
begin
  ...
  begin
    ...
    // Skip functions
    lastDir := ExtractLastDirectory(Copy(line, 1, LastDelimiter(PathDelim, line)));
    if lastDir <> 'functions' then
...
begin
  ...
  // NOTE: semantics are added per namespace
  {*
...
begin
  ...
  cppTxt.Add('');
  cppTxt.Add('void on_begin_run(void *userData); // USER');
  for i := 0 to GState.Effect.Passes.Count - 1 do
  begin
...
function FindNonCommentText(const Text: string; const SubStr: string; StartIndex: Integer = 1): Integer;
var
...
begin
  ...
  for i := 0 to NS.Resources.Count - 1 do
    cppTxt.Add(GState.CppTranslator.UResourceClass(NS.Resources[i].GetHLSLTypeString) + ' *' + CppFinalizeVariableName(NS.Resources[i]) + ' = nullptr;');
end;
...
begin
  ...
  try
    ...
    for i := 0 to NS.Structures.Count - 2 do
      ...
      begin
        if FindNonCommentText(NS.Structures[i].Text, NS.Structures[j].Name) <> 0 then
        begin
    ...
    for i := 0 to NS.ConstantBuffers.Count - 2 do
      ...
      begin
        if FindNonCommentText(NS.ConstantBuffers[i].Text, NS.ConstantBuffers[j].Name) <> 0 then
        begin
      ...
      case p.PassType of
        ...
        ptGeometry:
          begin
            ...
            passReflection.GS := TDFXShaderReflectionData.Create;
            try
              // TODO: check if shader profile is vaild
              ...
              if p.GeometryShader.Reference.Length > 0 then
              begin
                WriteShader(p.GeometryShader, hlslInclusionList, passReflection.GS, True);
                RebaseInclusions(hlslInclusionList, ExtractFilePath(GState.Effect.Parent.FindGlobalFunctionReference(p.GeometryShader.Reference).DiskPath),
                 GState.Effect.DiskPath);
                MergeStringLists(hlslInclusionList, finalHLSLInclusions);
              end;
            except
...
begin
  ...
  begin
    ...
    if Result[i+1] <> ':' then
    begin
      // Is this inside a define?
      Result[i] := '/';
      Insert('; /', Result, i);
    end
    else
      Inc(i); // Skip namespace

    i := FindNonCommentText(Result, ':', i + 1);
  end;
...
begin
  ...
  try
    ...
    sl.Add('#else');
    if Assigned(GState.CppTranslator) then
      sl.Add('extern ' +GState.CppTranslator.UResourceClass(rts) + ' *g_' + R.Name + ';');
    if R.ForceRegister <> -1 then
      sl.Add(Format('const uint32_t kRegister_%s = %d;', [R.Name, R.ForceRegister]));
    if R.Semantic.Length > 0 then
      sl.Add(Format('extern const char *kSemantic_%s;', [R.Name]));
    sl.Add('#endif');
    sl.EndNamespace;
    CppNamespaceSuffix(sl);
...
{*
procedure CompileDFXShader(Shader: TDFXFunction);
var
  i: Integer;
  func: TDFXShaderFunction;
  inc: TStringList;
begin
  ...
  try
    func.Reference := Shader.FilePath('::');
    func.Attributes := '';
    GState.CppTranslator := nil;
    GState.Effect := GState.Project.RootNamespace;
    GState.Handler.OnLog('Compiling ' + func.Reference);
    WriteShader(func, inc, nil, True);
    GState.Handler.OnProgress;
  finally
...
procedure CopyBuildDifferences(out NumFilesChanged: Integer;
  out NumFilesUnchanged: Integer; out NumFilesCopied: Integer);
var
  sourceLen: Integer;
  source, dest, path, mirrorPath: string;
  pred: TDirectory.TFilterPredicate;
  sl0, sl1: TStringList;
begin
  source := GState.BuildDir;
  sourceLen := source.Length + 1;
  dest := GState.OutputDir + 'dfx' + PathDelim;
  ...
  try
    ...
    // Copy changes
    for path in TDirectory.GetFiles(source, TSearchOption.soAllDirectories, pred)  do
    begin
      // Check files only
      if FileExists(path) then
      begin
        mirrorPath := dest + Copy(path, sourceLen, path.Length);

        if FileExists(mirrorPath) then
        begin
          sl0.LoadFromFile(path);
          sl1.LoadFromFile(mirrorPath);

          if (sl0.Text.Length <> sl1.Text.Length) or (sl0.Text <> sl1.Text) then
          begin
            sl0.SaveToFile(mirrorPath);
            Inc(NumFilesChanged);
          end
          else
            Inc(NumFilesUnchanged);
        end
        else
        begin
          ForceDirectories(ExtractFilePath(mirrorPath));
          CopyFileW(PChar(path), PChar(mirrorPath), True);
          Inc(NumFilesCopied);
        end;
      end;
    end;
...
procedure InternalCompileDFXProject(Project: TDFXProject; Handler: IDFXCompilerHandler;
  SkipEffects: Boolean);
var
  ns: TDFXNamespace;
  ...
  numFilesChanged, numFilesUnchanged, numFilesCopied: Integer;
begin
  ...
  try
    try
      ...
      GState.BuildDir := GState.OutputDir + 'dfx_build' + PathDelim;
      ...
      GState.Handler.OnLog('Merging differences...');
      GState.Handler.OnStatus('Merging differences...');
      CopyBuildDifferences(numFilesChanged, numFilesUnchanged, numFilesCopied);
      GState.Handler.OnLog(Format('Merge succeeded: %d changed, %d unchanged, %d copied.', [numFilesChanged, numFilesUnchanged, numFilesCopied]));

      GState.Handler.OnLog(StringOfChar('-', 50));
      GState.Handler.OnLog('Build succeeded.');
    except
      GState.Handler.OnLog(StringOfChar('-', 50));
...
procedure LoadCSO(const FileName: string; ShaderBC: ID3DBlob);
var
  fs: TFileStream;
...
begin
  ...
  try
    ...
    fs.ReadData(buf, fs.Size);
    if Failed(D3DCreateBlob(fs.Size, ShaderBC)) then
      raise Exception.Create('Could not open shader CSO file.');

    p := Pointer(ShaderBC.GetBufferPointer);
    Move(buf, p, ShaderBC.GetBufferSize);
  finally
    fs.Free;
  end;
end;
...
procedure CompileDFXProject(Project: TDFXProject; Handler: IDFXCompilerHandler);
begin
  InternalCompileDFXProject(Project, Handler, False);
end;

function GetShaderDASM(Project: TDFXProject; Handler: IDFXCompilerHandler;
  F: TDFXFunction; var DASM: string): Boolean;
var
  shaderBC, disassembly: ID3DBlob;
begin
  InternalCompileDFXProject(Project, Handler, True);
  GState.Handler.OnStatus('Disassembling ' + F.Name + '...');
  LoadCSO(F.DiskPath, shaderBC);

  if Failed(D3DDisassemble(shaderBC.GetBufferPointer, shaderBC.GetBufferSize, 0, nil, disassembly)) then
    Exit(False);

  DASM := PAnsiChar(disassembly.GetBufferPointer);
end;
