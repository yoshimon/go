...
/*!
    \file go/world/scene_manager.h
    \brief Contains a class for managing multiple scenes.
...
class GO_API scene_manager
{
    GO_DECLARE_NON_COPYABLE(scene_manager)
public:
    typedef std::vector<std::unique_ptr<scene>> scene_vector_t;
public:
    /*!
        Constructor.
     ...
     */
    scene_manager(go::cdefault_value);
    //! \overload
    scene_manager() = default;
    //! \overload
    scene_manager(const gfx_asset_context_parameters &assetContextParameters) { reset(assetContextParameters); }
public:
    /*!
        Resets the object.
